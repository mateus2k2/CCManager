...
ac:getHeight())end;bc:show()end}end}end;aa["libraries"]={}
...
db:setBackground(false,"\140",colors.black)local ac="horizontal"local bc=" "local cc=colors.gray;local dc=colors.gray;local _d=" "
local ad=colors.black;local bd=12;local cd=1;local dd=1
local function __a(b_a,c_a,d_a,_aa)local aaa,baa=b_a:getPosition()
local caa,daa=b_a:getSize()local _ba=ac=="vertical"and daa or caa
for i=0,_ba do
if

( (ac=="vertical"and
baa+i==_aa)or(ac=="horizontal"and aaa+i==d_a))and(aaa<=d_a)and(aaa+caa>d_a)and(baa<=_aa)and
(baa+daa>_aa)then cd=math.min(i+1,_ba- (#
bc+dd-2))
b_a:setValue(bd/_ba*cd)b_a:updateDraw()end end end
local a_a={getType=function(b_a)return _c end,load=function(b_a)b_a:listenEvent("mouse_click")
b_a:listenEvent("mouse_drag")b_a:listenEvent("mouse_scroll")end,setSymbol=function(b_a,c_a)
bc=c_a:sub(1,1)b_a:updateDraw()return b_a end,getSymbol=function(b_a)return bc end,setIndex=function(b_a,c_a)
cd=c_a;if(cd<1)then cd=1 end;local d_a,_aa=b_a:getSize()
cd=math.min(cd,(
ac=="vertical"and _aa or d_a)- (dd-1))
b_a:setValue(bd/ (ac=="vertical"and _aa or d_a)*cd)b_a:updateDraw()return b_a end,getIndex=function(b_a)return
cd end,setMaxValue=function(b_a,c_a)bd=c_a;return b_a end,getMaxValue=function(b_a)return bd end,setSymbolColor=function(b_a,c_a)ad=c_a
b_a:updateDraw()return b_a end,getSymbolColor=function(b_a)return ad end,setBarType=function(b_a,c_a)ac=c_a:lower()
b_a:updateDraw()return b_a end,getBarType=function(b_a)return ac end,mouseHandler=function(b_a,c_a,d_a,_aa)if
(db.mouseHandler(b_a,c_a,d_a,_aa))then __a(b_a,c_a,d_a,_aa)return true end;return
false end,dragHandler=function(b_a,c_a,d_a,_aa)if
(db.dragHandler(b_a,c_a,d_a,_aa))then __a(b_a,c_a,d_a,_aa)return true end;return
false end,scrollHandler=function(b_a,c_a,d_a,_aa)
if
(db.scrollHandler(b_a,c_a,d_a,_aa))then local aaa,baa=b_a:getSize()cd=cd+c_a;if(cd<1)then cd=1 end
cd=math.min(cd,(
ac=="vertical"and baa or aaa)- (dd-1))
b_a:setValue(bd/ (ac=="vertical"and baa or aaa)*cd)b_a:updateDraw()return true end;return false end,draw=function(b_a)
db.draw(b_a)
b_a:addDraw("slider",function()local c_a,d_a=b_a:getSize()local _aa,aaa=b_a:getPosition()
local baa,caa=b_a:getBackground(),b_a:getForeground()
if(ac=="horizontal")then b_a:addText(cd,aaa,bc:rep(dd))
if(dc~=false)then b_a:addBG(cd,1,ab[dc]:rep(
#bc*dd))end;if(cc~=false)then
b_a:addFG(cd,1,ab[cc]:rep(#bc*dd))end end
if(ac=="vertical")then
for n=0,d_a-1 do
if(cd==n+1)then for curIndexOffset=0,math.min(dd-1,d_a)do
b_a:addBlit(1,1 +n+curIndexOffset,bc,ab[ad],ab[ad])end else
if
(n+1 <cd)or(n+1 >cd-1 +dd)then b_a:addText(1,1 +n,_d)if(baa~=false)then
b_a:addBG(1,1 +n,ab[baa])end
if(caa~=false)then b_a:addFG(1,1 +n,ab[caa])end end end end end end)end}a_a.__index=a_a;return setmetatable(a_a,db)end end
...
db.customEventHandler(d_a,_aa,...)if _aa=="basalt_FrameResize"then bd=true end end,removeChild=function(d_a,_aa)if(
type(_aa)=="string")then _aa=d_a:getChild(_aa)end;if
(_aa==nil)then return end;db.removeChild(d_a,_aa)for aaa,baa in pairs(_d)do
if baa:getName()==
_aa:getName()then table.remove(_d,aaa)break end end;bd=true
...
if
(fs.exists(aba))then local bba=ab.loadBIMG(aba)print(bba[1][1])sleep(1)
ad=bb(bba)__a=1;bd=ad.getFrameObject(1)cd=ad.createBimg()
dd=bd.getImage()caa()_ba:updateDraw()end;return _ba end,setPath=function(_ba,aba)return
...
cba.zIndex>dba.zIndex end end;local function a_a(cba)cba:sortChildren()return bc end;local function b_a(cba,dba)
for _ca,aca in ipairs(bc)do if
aca.element:getName()==dba then return aca.element end end end
local function c_a(cba,dba)
local _ca=cba:getChild(dba)if(_ca~=nil)then return _ca end;for aca,bca in ipairs(bc)do
if(bca:getType()=="Container")then
local cca=bca:getDeepChild(dba)if(cca~=nil)then return cca end end end end
...
for _ca,aca in ipairs(bc)do if
aca.element:getName()==dba:getName()then table.remove(bc,_ca)cba:removeEvents(dba)ad=false
cba:updateDraw()return true end end;return false end;local function aaa(cba)local dba=cba:getParent()bc={}cc={}ad=false;bd=0;cd=0;_d=nil
...
cc=os.startTimer(_c)end end end}_d.__index=_d;return setmetatable(_d,cb)end end;return aa["main"]()